...
const props = defineProps({
  ...
  // The admin sees how many are in stock, customers only see whether it is in stock
...
<template>
  <div
    class="flex w-52 max-w-52 flex-col items-center rounded-xl border border-blue-200/80 bg-blue-100/80 p-2 shadow-sm shadow-blue-900/5 transition-[background-color,border-color,box-shadow] duration-300 ease-out hover:border-blue-300 hover:bg-blue-100 hover:shadow-lg hover:shadow-blue-900/10"
  >
    <h2 class="mt-2 mb-1 h-10 max-h-10 overflow-hidden text-center text-sm font-semibold">
      {{ productDetails.productName }}
    </h2>
    <div class="flex h-60 max-h-60 w-40 max-w-40 justify-center rounded-lg bg-white/70 p-2">
      <div
    ...
    <div class="text-center">
      ...
      <div>
        <span v-if="showStockCount" class="text-sm">
          {{
            productDetails.productStock > 0
              ? `${productDetails.productStock} in stock`
              : 'Out of stock'
          }}
        </span>
        <span v-else class="text-sm">{{ productDetails.productStockStatus }}</span>
      </div>
    </div>
  </div>
</template>
